//same implementation using class in JS
...
class Book{  //creating a book class with constructor
    constructor(name, author, type){
        this.name=name;  this.author=author;  this.type=type;
    }
};

class Display{  //creating a display class with functions
    clear(){  //function to clear the form after submiting it
    ...
    validate(book){  //function to check if book entered is valid or not
        if(book.name.length<2 || book.author.length<2 || book.type.length<2){ return false; }
        else{ return true; }
    }
    showmessage(type, displaymessage){  //function show messages
...
Array.from(radio).forEach( function(index){  //traversing each button as array
    ...
    // console.log(radio_button_id);  //for checking button id
